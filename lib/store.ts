...
export type Appt = {
  ...
  phone: string;
  age: number;
  gender: "M" | "F" | null; // null = not collected (e.g. bot bookings, which never ask)
...
function seed(): Appt[] {
  ...
  const rows: [string, string, number, Source, ApptStatus, boolean, "cash" | null, "M" | "F"][] = [
    ["Lakshmi Devi", "9848012345", 62, "website", "done", true, "cash", "F"],
    ["Ravi Teja", "9701123456", 28, "whatsapp", "done", true, "cash", "M"],
    ["Suresh Kumar", "9885234567", 55, "walkin", "done", true, "cash", "M"],
    ["Anjali Rao", "9963345678", 41, "website", "consulting", true, "cash", "F"],
    ["Md. Imran", "9848456789", 35, "whatsapp", "waiting", false, null, "M"],
    ["Padma Sri", "9701567890", 23, "walkin", "waiting", false, null, "F"],
    ["Venkata Rao", "9885678901", 78, "website", "waiting", false, null, "M"],
    ["Kavya Reddy", "9963789012", 32, "whatsapp", "reserved", false, null, "F"],
    ["Ganesh Babu", "9848890123", 45, "walkin", "reserved", false, null, "M"],
    ["Sita Mahalakshmi", "9701901234", 68, "website", "reserved", false, null, "F"],
  ];
...
// Combined age/gender display, e.g. " · 32/M". Hides either part when absent
// (age 0 = never collected; gender null = never collected, e.g. bot bookings).
export function ageGenderLabel(a: { age: number; gender: "M" | "F" | null }): string {
  if (a.age > 0 && a.gender) return ` · ${a.age}/${a.gender}`;
  if (a.age > 0) return ` · Age ${a.age}`;
  if (a.gender) return ` · ${a.gender}`;
  return "";
}
